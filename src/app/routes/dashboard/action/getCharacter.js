...
console.log(sessionStorage.getItem('CharacterName'));
  ...
  request('GET', walletURL, {json: true}).done((result) => {
    if(result.statusCode == 200){
      ...
      dispatch({ type: 'SHOW_CHARACTER_WALLET', payload: wallet })
    } else {
  ...
  request('GET', ordersURL, {json: true}).done((result) => {
    if(result.statusCode == 200){
      ...
      dispatch({ type: 'SHOW_CHARACTER_ORDERS', payload: ordersArray })
    }
  })
  ...
  request('GET', jobsURL, {json: true}).done((result) => {
    if(result.statusCode == 200){
      ...
      dispatch({ type: 'SHOW_CHARACTER_JOBS', payload: jobsArray })
    }
  })
