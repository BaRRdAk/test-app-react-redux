...
export const importPriceHistory = (regionId) => dispatch => {
  ...
  dispatch({
    type: 'IMPORT_PRICE_HISTORY_CLEAN',
    payload: null
  })
      ...
      event.target.result.map((typeId) => {
        ...
        request('GET', url, {
          ...
          db.transaction(function(tx) {
            tx.executeSql(
              "CREATE TABLE IF NOT EXISTS PriceHistory (type_id REAL, region_id REAL, date REAL, order_count REAL, volume REAL, highest REAL, average REAL, lowest REAL)", [],
              null, null);
            tx.executeSql(
              "DELETE FROM PriceHistory WHERE type_id = ? AND region_id = ?", [
                typeId, regionId
              ], null, null);

            resArray.map((e) => {
              tx.executeSql(
                "INSERT INTO PriceHistory (type_id, region_id, date, order_count, volume, highest, average, lowest) values(?, ?, ?, ?, ?, ?, ?, ?)", [
                  typeId, regionId, e.date, e.order_count,
                  e.volume, e.highest, e.average, e.lowest
                ], null, null);
              dispatch({
                type: 'IMPORT_PRICE_HISTORY',
                payload: 1
              })
            });
